...
const Auth = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-secondary via-background to-secondary flex justify-center items-center flex-col font-sans py-8 px-4">
      {/* Logo */}
      ...
      {/* Container */}
      <div 
        className={`bg-card rounded-xl shadow-2xl relative overflow-hidden w-full max-w-[768px] min-h-[480px] transition-all duration-600 ${
      ...
      >
          ...
          <form onSubmit={handleSignUp} className="bg-card flex items-center justify-center flex-col px-8 md:px-12 h-full text-center">
            ...
            <div className="flex gap-3 my-4">
              <button 
                ...
                className="border border-border rounded-full w-10 h-10 flex items-center justify-center hover:bg-secondary transition-colors"
              >
                <Chrome className="w-5 h-5 text-muted-foreground" />
              </button>
              <button 
                type="button"
                className="border border-border rounded-full w-10 h-10 flex items-center justify-center hover:bg-secondary transition-colors opacity-50 cursor-not-allowed"
                disabled
              >
            ...
            <button 
              type="submit"
              disabled={isLoading}
              className="rounded-full border border-primary bg-primary text-primary-foreground text-xs font-bold py-3 px-11 uppercase tracking-wider mt-4 transition-transform active:scale-95 hover:bg-primary/90 disabled:opacity-50 flex items-center gap-2"
            >
        ...
        >
          <div 
            className={`bg-gradient-to-r from-primary to-accent text-primary-foreground relative -left-full h-full w-[200%] transition-transform duration-600 ease-in-out ${
              isRightPanelActive ? 'translate-x-1/2' : 'translate-x-0'
            }`}
          >
            {/* Left Panel */}
            <div 
              className={`absolute flex items-center justify-center flex-col px-8 text-center top-0 h-full w-1/2 transition-transform duration-600 ease-in-out ${
            ...
            >
              ...
              <button 
                ...
                className="rounded-full bg-transparent border border-white text-white text-xs font-bold py-3 px-11 uppercase tracking-wider transition-all hover:bg-white/10 active:scale-95"
              >
                Masuk
              </button>
            </div>
            ...
            {/* Right Panel */}
            <div 
              className={`absolute flex items-center justify-center flex-col px-8 text-center top-0 h-full w-1/2 right-0 transition-transform duration-600 ease-in-out ${
      ...
      <button 
        onClick={() => navigate('/')}
        className="mt-6 text-muted-foreground hover:text-primary text-sm transition-colors"
      >
        ← Kembali ke Beranda
      </button>
